...
$("adminLoginForm").addEventListener("submit", async (e) => {
  e.preventDefault();
  const rawUsername = $("adminUsername").value;
  const username = rawUsername.trim().toLowerCase();
  const password = $("adminPassword").value;
  $("adminLoginNote").textContent = "";
  debug(`Typed username: "${rawUsername}" -> normalized: "${username}"`);
  debug(`Known usernames: ${Object.keys(USERNAME_TO_EMAIL).join(", ") || "(none — map is empty!)"}`);

  const email = USERNAME_TO_EMAIL[username];
  if (!email) {
    $("adminLoginNote").textContent = "Unknown username.";
    debug("No match found in USERNAME_TO_EMAIL for that key.");
    return;
  }
  debug(`Matched email: ${email}. Attempting sign-in...`);
  try {
    await signInWithEmailAndPassword(auth, email, password);
    debug("Sign-in call succeeded.");
  } catch (err) {
    $("adminLoginNote").textContent = "Invalid username or password.";
    debug(`Sign-in failed: ${err.code || ""} ${err.message || err}`);
  }
});
