...
// TEMPORARY: login screen is disabled. The page signs itself in with this
// fixed admin account on load, silently, so Firestore rules (which still
// require an authenticated admin) keep working. Put the login form back
// before sharing this page with anyone else — see README.
const ADMIN_EMAIL = "admin@liteware.me";
...
(async function autoSignIn() {
  try {
    ...
    debug(`Auto sign-in failed: ${err.code || ""} ${err.message || err}`);
  }
})();

onAuthStateChanged(auth, async (user) => {
  if (!user) {
    debug("Not signed in yet...");
    return;
  }
  const claimSnap = await getDoc(doc(db, "admins", user.uid));
  if (!claimSnap.exists()) {
    debug(`Signed in as ${user.uid} but no admins/${user.uid} document exists — create it in Firestore.`);
    return;
  }
  show("adminScreen");
  await loadUsers();
